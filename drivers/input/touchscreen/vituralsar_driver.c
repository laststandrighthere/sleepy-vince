...
static int sar_parse_dt(struct device *dev)
{
	...
	if (sar_int_gpio == 130) {
		pr_debug("parse irq gpio correctly\n ");
		return 0;
	} else {
		pr_debug("parse irq gpio incorrectly\n ");
		return -1;
	}
}
...
static irqreturn_t sar_irq_handler(int irq, void *dev_id)
{
	...
	pr_debug("input_report_key ,status :%d\n",input_data);

	input_sync(sdata->input_dev);
	spin_unlock_irqrestore(&sdata->irq_lock, irqflags);

	pr_debug("gpio_get_value : %d\n",input_data);
...
static s8 sar_request_irq(struct vituralsar_data *sdata)
{
	...
	ret = request_irq(sdata->client->irq,
			...
			sdata);
	if (ret) {
		 pr_debug("Request IRQ failed!ERRNO:%d.", ret);
	} else {
		 sdata->use_irq = 1;
		 return 0;
	}
...
static s8 sar_request_io_port(struct vituralsar_data *sdata)
{
	...
	if (ret < 0) {
		pr_debug("Failed to request GPIO:%d, ERRNO:%d",
				(s32)sar_int_gpio, ret);
...
static s8 sar_request_input_dev(struct vituralsar_data *sdata)
{
	...
	if (ret) {
		 pr_debug("Register %s input device failed", sdata->input_dev->name);
		 return -ENODEV;
	}
		...
		const struct i2c_device_id*id)
{
	...
	if (sdata == NULL) {
	 	pr_debug("Alloc GFP_KERNEL memory failed.");
			return -ENOMEM;
	}
	if (client->dev.of_node) {
		ret = sar_parse_dt(&client->dev);
		if (!ret)
			pr_debug("sar_parse_dt success\n");
	}
	...
	if (gpio_is_valid(sar_int_gpio)) {
		...
		if (ret < 0) {
			pr_debug("SAR %s -request io port fail\n",__func__);
			return -ENOMEM;
		}
	} else {
		pr_debug("SAR %s -gpio is not valid\n",__func__);
		return -ENOMEM;
	}

	ret = sar_request_input_dev(sdata);
	if (ret < 0)
		pr_debug("SAR request input dev failed");

	ret = sar_request_irq(sdata);
	if (ret < 0)
		pr_debug("SAR %s -request irq fail\n",__func__);

	__set_bit(EV_REP, sdata->input_dev->evbit);
	if (sdata->use_irq)
		sar_irq_enable(sdata);

	pr_debug("after sar_irq_enable,probe end \n");

	gpio_status = proc_create(GOIP_STATUS, 0644, NULL, &gpio_status_ops);
	if (gpio_status == NULL)
		pr_debug("tpd, create_proc_entry gpio_status_ops failed\n");
